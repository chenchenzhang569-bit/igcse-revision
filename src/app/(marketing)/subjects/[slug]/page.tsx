...
import { getSubjectData, type Topic } from "@/lib/subject-data";

function TopicGrid({ slug, topics }: { slug: string; topics: Topic[] }) {
  return (
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 sm:py-12">
      ...
      <section className="mt-8">
        ...
        <TopicGrid slug={slug} topics={topics} />
      </section>

      <div className="flex gap-3 flex-wrap mt-8 pt-6 border-t">
        <Link href={`/past-papers/${slug}`} className="bg-primary-900 text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-primary-800 transition">
          📄 Past Papers
        </Link>
        <Link href={`/mock-exams/${slug}`} className="bg-accent-500 text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-accent-600 transition">
          📝 Mock Exams
        </Link>
        <Link href={`/subjects?board=${board}`} className="bg-gray-100 text-primary-900 px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-gray-200 transition">
          📚 All {board} Subjects
        </Link>
      </div>
    </div>
