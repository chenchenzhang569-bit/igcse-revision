...
const NAMES: Record<string, string> = {
  physics: "Physics", chemistry: "Chemistry", biology: "Biology", mathematics: "Mathematics",
};
...
export default async function SubjectPage({
  ...
  if (!parsed) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center">
        <p className="text-gray-400 text-lg">Subject not found (slug: {slug})</p>
  ...
  const topics = ALL_TOPICS[subjectSlug] || [];
  const icon = ICONS[subjectSlug] || "📚";
  const displayName = NAMES[subjectSlug] || subjectSlug;
      ...
      <div className="flex items-center gap-4 mt-4">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-primary-900">
            {board} IGCSE {displayName}
          </h1>
          <p className="text-gray-500 text-xs">
            slug={slug} | parsed={subjectSlug} | topics={topics.length} | v4
          </p>
        </div>
      </div>
      ...
      <section className="mt-8">
        ...
        {topics.length > 0 ? (
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-sm text-yellow-800">
            <p className="font-semibold">No topics for: &quot;{subjectSlug}&quot;</p>
            <p className="mt-1">Available keys: {JSON.stringify(Object.keys(ALL_TOPICS))}</p>
          </div>
        )}
      </section>
