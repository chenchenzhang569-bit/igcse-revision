...
export async function GET(request: NextRequest) {
  ...
  // Get subject names + boards
  let subjectMap: Record<string, { name: string; code: string; board: string }> = {};
  if (allSubjectIds.length > 0) {
    const [subRes, boardRes] = await Promise.all([
      fetch(`${API}/subjects?select=id,display_name,code,exam_board_id&id=in.(${allSubjectIds.join(",")})`, { headers: getSrHeaders() }),
  ...
  if (hasAllSubjectPlan) {
    ...
    const resAll = await fetch(
      `${API}/subjects?select=id,display_name,code,exam_board_id&is_published=eq.true`,
      { headers: getSrHeaders() }
    );
    ...
    if (Array.isArray(allSubjects) && allPlanPurchase) {
      for (const s of allSubjects) {
        ...
        subjectMap[s.id] = { name: s.display_name || s.code, code: s.code || "", board: "" };
        purchases.push({
