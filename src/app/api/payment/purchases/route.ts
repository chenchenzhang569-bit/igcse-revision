    ...
    72|      fetch(`${API}/subjects?select=id,display_name,code,exam_board_id&id=in.(${allSubjectIds.join(",")})`, { headers: getSrHeaders() }),
    ...
    94|      `${API}/subjects?select=id,display_name,code,exam_board_id&is_published=eq.true`,
   ...
   101|        subjectMap[s.id] = { name: s.display_name || s.code, code: s.code || "", board: "" };
   102|        purchases.push({
